...
public class MagicalDamageDealt : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		if (other.gameObject.tag == "Enemy") {
			...
			if (weap.keyDelay == true) {
				...
				//Calculate Damage
				calculateDamage = playerStat.totalMagicDam;

				if (skill.focusActive) {
					calculateDamage = calculateDamage * 2;
				}

				chance = Random.Range (0, playerStat.magicCrit);
				if (chance < playerStat.magicCrit) {
					calculateDamage = calculateDamage * 2;
				}
